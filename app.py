...
app.secret_key = 'your-very-secure-secret-key-here'  # Change this to a strong secret!
app.config['LOGIN_DETAILS'] = {
    'patient': {'username': 'patient1', 'password': 'patient123456'},
    'doctor': {'username': 'doctor', 'password': 'doctor123456'}
}
...
class Patient(db.Model):
    ...
    plot_filename = db.Column(db.String(120))  # Add this line
...
def patient_login():
    if request.method == 'POST':
        ...
        if (username == app.config['LOGIN_DETAILS']['patient']['username'] and 
            password == app.config['LOGIN_DETAILS']['patient']['password']):
...
def doctor_login():
    if request.method == 'POST':
        ...
        if (username == app.config['LOGIN_DETAILS']['doctor']['username'] and 
            password == app.config['LOGIN_DETAILS']['doctor']['password']):
...
def submit():
    try:
        ...
        try:
            ...
            # Create filename using patient ID
            plot_filename = f'ecg_plot_{new_patient.id}.png'
            plot_path = os.path.join(plots_dir, plot_filename)
            
            # Create the plot
            plt.figure(figsize=(12, 4))
            plt.plot(ecg_normalized[:1000], color='#2c7be5')  # Plot first 1000 samples
            plt.title(f'ECG Signal - {name}', fontsize=14)
            plt.xlabel('Samples (360 Hz)', fontsize=12)
            ...
            # Update patient with plot filename
            new_patient.plot_filename = f'plots/{plot_filename}'
            db.session.commit()
            
        except Exception as e:
            app.logger.error(f"ECG plot error: {str(e)}")
            new_patient.plot_filename = None
            db.session.commit()

        flash(f'ECG submitted successfully! Processed {len(beats)} beats')
        return redirect(url_for('patient_form'))
